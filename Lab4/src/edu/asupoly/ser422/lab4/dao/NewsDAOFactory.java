...
public final class NewsDAOFactory {
	...
	public static final INewsDAO getTheDAO() {
		if (__theDAO == null) {
			...
			InputStream is = null;
			try {
				is = NewsDAOFactory.class.getClassLoader().getResourceAsStream("dao.properties");
				if (is != null) {
					Properties p = new Properties();
					p.load(is);
					String daoClassName = p.getProperty("dao.class");
					if (daoClassName.equals("edu.asupoly.ser422.lab4.dao.NewsPeriodicDAO")) {
				...
				t.printStackTrace();
				__theDAO = new NewsDefaultDAO();
				System.out.println("Problem creating DAO, instead reated DAO: NewsDefaultDAO");
			}
		}
		return __theDAO;
	}
